...
#
cat dodatkowe/nowomowa.txt | awk '{chars+=length($0)+1; words+=NF; lines+=1} END {print chars "\n" words "\n" lines}'
# dla każdej linii dodajemy do zmiennych odpowiednie wartości
# na końcu wypisujemy wartości zmiennych
# length($0) zwraca długość linii, NF zwraca liczbę słów
# do length($0) dodajemy 1, aby uwzględnić znak końca linii!
